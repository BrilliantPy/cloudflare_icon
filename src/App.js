...
function App() {
  ...
  const copytoClipboard = ()=>{
    navigator.clipboard.writeText(getFullCode(selectIcons))
    setCopyStatus('Copied')
  }

  const getFullCode = (e)=>{
    const fullCode = `<i `+typeClass+`="fa `+e+` fa-fw"></i>`
    return fullCode
  }
   
  return (
    <div className="container-fluid" style={{color:"#0d6efd"}}>
      <div className="headContainer">
        <header>
          <h1 className='center'>Cloudflare Icon Selector</h1>
        </header>
      </div>  
      
      <div className='container-fluid'>
          <div className="row">
            <div className="col-md-2"><b>Mode: </b><br></br>
              <div className='radioMode'>
                <div className='modeLeft' onClick={()=>{
                  setMode("REACT")
                  setTypeClass("className")
                  }}>
                  <label className="container">
                  <input type="radio" name="radio" value={modes} checked={modes=="REACT"}/>
                  <span className="checkmark"></span>
                  React</label>                  
                </div>

                <div className='modeRight' onClick={()=>{
                  setMode("HTML")
                  setTypeClass("class")
                  }}>
                  <label className="container">
                  <input type="radio" name="radio" value={modes} checked={modes=="HTML"}/>
                  <span className="checkmark"></span>HTML
                  </label>
                </div>
              </div>
              <div className="codeContainer">
                <code id="fullCode">{getFullCode(selectIcons)}</code>
              </div>
            ...
            <div className="col-md-10">
              <ul>
                {data.map(e => {  
                  const fullClassName = "fa "+e+" fa-fw"
                  return (<li className='groupContent'><button className='btn btn-outline-primary hoverZoom' onClick={()=>selectIcon(e)} title={e}><i className={fullClassName}></i></button></li>)
                })}  
              </ul>
            </div>
          </div>
      </div>
      <footer className="bp-center bp-margin-top">
        <p>Copyright 2021 © BrilliantPy</p>
      </footer>
    </div>
  );
}
